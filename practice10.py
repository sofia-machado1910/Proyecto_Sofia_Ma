...
def cargar_datos():
    ...
    for equipo in data_equipos:
        equipos.append(Equipo(equipo["name"], equipo["code"], equipo["group"]))  
    ...
    for estadio in data_estadios:
        estadios.append(Estadio(estadio["name"], estadio["city"]))
    ...
    for partido in data_partidos:
        equipo_local = next((e for e in equipos if e.code == partido["home"]), None)
        equipo_visitante = next((e for e in equipos if e.code == partido["away"]), None)
        estadio = next((s for s in estadios if s.nombre == partido["stadium_id"]), None)
        partidos.append(Partido(equipo_local, equipo_visitante, partido["date"], partido["hora"], estadio))
...
def gestionar_partidos_estadios(equipos, estadios, partidos):
    while True:
        ...
        if opcion == "1":
            ...
            partido = next((p for p in partidos if p.equipo_local.code == codigo_partido or p.equipo_visitante.code == codigo_partido), None)
            if partido:
...
def gestionar_venta_entradas(equipos, estadios, partidos):
    ...
    while True:
        ...
        if opcion == "1":
            ...
            cedula = input("Ingrese la cédula del cliente: ")
            entrada = next((e for e in entradas if e.partido.equipo_local.code == cedula or e.partido.equipo_visitante.code == cedula), None)
            if entrada:
...
def gestionar_asistencia_partidos(entradas):
    ...
    while True:
        ...
        if opcion == "1":
            codigo_partido = input("Ingrese el código del partido: ")
            entrada = next((e for e in entradas if e.partido.equipo_local.code == codigo_partido or e.partido.equipo_visitante.code == codigo_partido), None)
            if entrada:
...
def indicadores_gestion(equipos, estadios, partidos, clientes, entradas, ventas):
    while True:
        ...
        if opcion == "1":
            ...
            for entrada in entradas:
                cliente = next((c for c in clientes if c.cedula == entrada.partido.equipo_local.code or c.cedula == entrada.partido.equipo_visitante.code), None)
                if cliente:
