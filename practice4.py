...
def gestionar_restaurantes_vendedores():
    ...
    if 1 <= opcion_restaurante <= len(restaurantes):
        ...
        while True:
            ...
            #     stock = obtener_stock_producto(estadio_nombre, restaurante, nombre_producto)

            #     if stock is not None:
            #         print(f"El stock actual de {nombre_producto} es: {stock}")
            #     else:
            #         print(f"No se encontró el producto {nombre_producto} en este restaurante.")
            elif opcion_productos == "5":
                break
...
def crear_base_de_datos():
    ...
    # Check if the "usado" column exists
...
def calcular_promedio_gasto_vip():
    """
    Calcula el promedio de gasto de los clientes VIP en un partido (ticket + restaurante).
    """
    gastos_vip = []
    for entrada in entradas:
        if entrada.tipo == "VIP":
            gastos_vip.append(entrada.costo)
            # Buscar órdenes del restaurante asociadas a la entrada VIP
            for orden in ordenes_restaurante:
                if orden.cliente.cedula == entrada.cliente.cedula:
                    gastos_vip[-1] += orden.costo
    promedio = sum(gastos_vip) / len(gastos_vip) if gastos_vip else 0
    return promedio

def obtener_estadísticas_partidos():
    """
    Obtiene estadísticas de partidos ordenadas por asistencia.
    """
    partidos_ordenados = sorted(partidos, key=lambda partido: partido.asistencia, reverse=True)
    estadísticas_partidos = []
    for partido in partidos_ordenados:
        ...
        if equipo_local is None:
            equipo_local_nombre = "Equipo no encontrado" 
        else:
            equipo_local_nombre = equipo_local.nombre

        if equipo_visitante is None:
            equipo_visitante_nombre = "Equipo no encontrado" 
        else:
            equipo_visitante_nombre = equipo_visitante.nombre

        estadio_nombre = obtener_nombre_estadio_por_id(partido.estadio)
        relacion_asistencia_venta = partido.asistencia / partido.asistencia if partido.asistencia else 0
        estadísticas_partidos.append({"partido": f"{equipo_local_nombre} vs {equipo_visitante_nombre}", 
                                     "estadio": estadio_nombre,
                                     "boletos_vendidos": partido.asistencia,
                                     "personas_asistiendo": partido.asistencia,
                                     "relacion_asistencia_venta": relacion_asistencia_venta})
    return estadísticas_partidos

def obtener_partido_con_mayor_asistencia():
    """
    Obtiene el partido con mayor asistencia.
    """
    partido_mayor_asistencia = max(partidos, key=lambda partido: partido.asistencia)
    return partido_mayor_asistencia
...
def obtener_partido_con_mayor_boletos_vendidos():
    """
    Obtiene el partido con mayor cantidad de boletos vendidos.
    """
    partido_mayor_boletos_vendidos = max(partidos, key=lambda partido: partido.asistencia)
    return partido_mayor_boletos_vendidos
...
def obtener_productos_mas_vendidos():
    """
    Obtiene los 3 productos más vendidos en el restaurante.
    """
    productos_vendidos = {}
    for orden in ordenes_restaurante:
        for producto in orden.productos:
...
def obtener_clientes_con_mas_boletos():
    """
    Obtiene los 3 clientes que más boletos compraron.
    """
    clientes_boletos = {}
    for entrada in entradas:
        if entrada.cliente.cedula in clientes_boletos:
...
def mostrar_estadisticas():
    ...
    """
    console = Console()
    table = Table(title="Estadísticas de la Eurocopa Alemania 2024")
    table.add_column("Indicador", style="cyan", no_wrap=True)
    table.add_column("Valor", style="magenta")

    promedio_gasto_vip = calcular_promedio_gasto_vip()
    table.add_row("Promedio de gasto de clientes VIP", f"{promedio_gasto_vip:.2f}")

    estadisticas_partidos = obtener_estadísticas_partidos()
    table_partidos = Table(title="Asistencia a Partidos")
    table_partidos.add_column("Partido", style="cyan", no_wrap=True)
    table_partidos.add_column("Estadio", style="cyan", no_wrap=True)
    table_partidos.add_column("Boletos Vendidos", style="cyan", no_wrap=True)
    table_partidos.add_column("Personas Asistiendo", style="cyan", no_wrap=True)
    table_partidos.add_column("Relación Asistencia/Venta", style="cyan", no_wrap=True)

    for estadistica in estadisticas_partidos:
        table_partidos.add_row(estadistica["partido"],
                            estadistica["estadio"],
                            str(estadistica["boletos_vendidos"]),
                            str(estadistica["personas_asistiendo"]),
                            f"{estadistica['relacion_asistencia_venta']:.2f}")

    partido_mayor_asistencia = obtener_partido_con_mayor_asistencia()
    equipo_local = buscar_equipo_por_codigo_fifa(partido_mayor_asistencia.equipo_local)
    equipo_visitante = buscar_equipo_por_codigo_fifa(partido_mayor_asistencia.equipo_visitante)
    estadio_nombre = obtener_nombre_estadio_por_id(partido_mayor_asistencia.estadio)
    table.add_row("Partido con mayor asistencia", f"{equipo_local.nombre} vs {equipo_visitante.nombre} en {estadio_nombre}")

    partido_mayor_boletos_vendidos = obtener_partido_con_mayor_boletos_vendidos()
    equipo_local = buscar_equipo_por_codigo_fifa(partido_mayor_boletos_vendidos.equipo_local)
    equipo_visitante = buscar_equipo_por_codigo_fifa(partido_mayor_boletos_vendidos.equipo_visitante)
    estadio_nombre = obtener_nombre_estadio_por_id(partido_mayor_boletos_vendidos.estadio)
    table.add_row("Partido con mayor boletos vendidos", f"{equipo_local.nombre} vs {equipo_visitante.nombre} en {estadio_nombre}")

    productos_mas_vendidos = obtener_productos_mas_vendidos()
    table_productos = Table(title="Productos Más Vendidos")
    table_productos.add_column("Producto", style="cyan", no_wrap=True)
    table_productos.add_column("Unidades Vendidas", style="cyan", no_wrap=True)
    for i, producto in enumerate(productos_mas_vendidos):
        table_productos.add_row(producto[0], str(producto[1]))

    clientes_mas_boletos = obtener_clientes_con_mas_boletos()
    table_clientes = Table(title="Clientes con Más Boletos")
    table_clientes.add_column("Cédula", style="cyan", no_wrap=True)
    table_clientes.add_column("Boletos Comprados", style="cyan", no_wrap=True)
    for i, cliente in enumerate(clientes_mas_boletos):
        table_clientes.add_row(str(cliente[0]), str(cliente[1]))

    console.print(table)
    console.print(table_partidos)
    console.print(table_productos)
    console.print(table_clientes)
...
while True:
    ...
    print("4. Módulo de estadísticas.")
    print("5. Salir")
