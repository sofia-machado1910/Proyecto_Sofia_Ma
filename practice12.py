...
def vender_entrada(cliente, partido, tipo):
    ...
    mostrar_mapa_estadio(partido.estadio)

    while True:
        asiento_numero = int(input("Ingrese el número del asiento: "))
        asiento_fila = int(input("Ingrese el número de la fila: "))
        asiento = buscar_asiento_por_numero(asiento_numero, asiento_fila)
        if asiento is None:
    ...
    print(f"Entrada vendida correctamente.\n"
          f"Cliente: {cliente.nombre}\n"
          f"Partido: {partido.equipo_local} vs {partido.equipo_visitante}\n"
          f"Fecha y hora: {partido.fecha_hora}\n"
          f"Tipo de entrada: {tipo}\n"
          f"Asiento: Fila {asiento.fila}, Asiento {asiento.numero}\n"
          f"Subtotal: ${costo / 1.16:.2f}\n"
          f"IVA (16%): ${costo * 0.16:.2f}\n"
          f"Total: ${costo:.2f}")

    # Preguntar si desea pagar la entrada
...
def mostrar_informacion_partidos(partidos_filtrados):
    if partidos_filtrados:
        for i, partido in enumerate(partidos_filtrados):
            ...
            # Mostrar el estadio después de que el usuario seleccione un partido
            # seleccionar_partido = input("¿Desea ver el mapa del estadio de este partido? (s/n): ").lower()
            # if seleccionar_partido == "s":
            #     print_stadiums_from_api()
            #     # Encuentra el estadio en la lista de estadios y muestra su información
            #     for estadio in estadios:
            #         if estadio.nombre == estadio_nombre:
            #             # Implementar la lógica para mostrar la información del estadio
            #             print(f"Nombre del estadio: {estadio.nombre}")
            #             print(f"Ubicación: {estadio.ubicacion}")
            #             # ... (mostrar más información del estadio si se necesita)
            #             break
            #     else:
            #         print("No se encontró el estadio.")
    else:
...
# Menú principal
while True:
    print("\nBienvenido a la venta oficial de entradas para la Eurocopa Alemania 2024!")
    input("Para iniciar requeremos algunos de sus datos personales [Presione ENTER para continuar]")
    ...
    while True:
        ...
        if partidos_filtrados:
            ...
            if seleccionar_partido == "s":
                ...
                # Si no quiere seleccionar, vuelve al menú de búsqueda de partidos
                continue
        else:
