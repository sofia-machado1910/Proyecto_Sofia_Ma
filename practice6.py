...
def vender_entrada(cliente, partido, tipo):
    asiento = buscar_asiento_libre(partido)
    if asiento is None:
...
def crear_asiento(numero, fila, ocupado):
    asiento = Asiento(numero, fila, ocupado)
    asientos.append(asiento)
    guardar_asientos()
    return asiento

def buscar_asiento_libre(partido):
...
def cargar_partidos_desde_archivo(archivo):
    ...
    with open(archivo, "r") as f:
        ...
        for partido_json in partidos_json:
            ...
            partidos.append(Partido(partido_json["equipo_local"], partido_json["equipo_visitante"], fecha_hora, partido_json["estadio"]))
