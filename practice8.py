import requests
import json
import datetime
from collections import namedtuple
from functools import reduce
import matplotlib.pyplot as plt

class Equipo:
    def __init__(self, nombre, codigo_fifa, grupo):
        self.nombre = nombre
        self.codigo_fifa = codigo_fifa
        self.grupo = grupo

class Estadio:
    def __init__(self, nombre, ubicacion):
        self.nombre = nombre
        self.ubicacion = ubicacion

class Partido:
    def __init__(self, equipo_local, equipo_visitante, fecha_hora, estadio, asistencia=0):
        self.equipo_local = equipo_local
        self.equipo_visitante = equipo_visitante
        self.fecha_hora = fecha_hora
        self.estadio = estadio
        self.asistencia = asistencia

class Asiento:
    def __init__(self, numero, fila, estadio, ocupado=False):
        self.numero = numero
        self.fila = fila
        self.estadio = estadio
        self.ocupado = ocupado

class Cliente:
    def __init__(self, nombre, cedula, edad):
        self.nombre = nombre
        self.cedula = cedula
        self.edad = edad

class Entrada:
    def __init__(self, cliente, partido, tipo, asiento, costo):
        self.cliente = cliente
        self.partido = partido
        self.tipo = tipo
        self.asiento = asiento
        self.costo = costo

class Producto:
    def __init__(self, nombre, tipo, precio, inventario=10):
        self.nombre = nombre
        self.tipo = tipo
        self.precio = precio
        self.inventario = inventario

class OrdenRestaurante:
    def __init__(self, cliente, productos, costo):
        self.cliente = cliente
        self.productos = productos
        self.costo = costo

# Funciones auxiliares para validar números vampiro y números perfectos
def es_numero_vampiro(cedula):
    cedula_str = str(cedula)
    largo = len(cedula_str)
    if largo % 2 != 0:
        return False
    mitad1 = cedula_str[:largo // 2]
    mitad2 = cedula_str[largo // 2:]
    for divisor in range(1, int(mitad1) + 1):
        if divisor * int(mitad2) == cedula and divisor != 1 and divisor != int(mitad1):
            return True
    return False

def es_numero_perfecto(numero):
    if numero <= 1:
        return False
    divisores = [1]
    for i in range(2, numero):
        if numero % i == 0:
            divisores.append(i)
    suma_divisores = reduce(lambda x, y: x + y, divisores)
    return suma_divisores == numero

# Funciones para cargar datos desde la API
def cargar_equipos():
    equipos_json = requests.get("https://raw.githubusercontent.com/Algoritmos-y-Programacion/api-proyecto/main/teams.json").json()
    equipos = []
    for equipo_json in equipos_json:
        nombre = equipo_json["name"]
        codigo_fifa = equipo_json["fifa_code"]
        grupo = equipo_json["group"]
        equipos.append(Equipo(nombre, codigo_fifa, grupo))
    return equipos

def cargar_estadios():
    estadios_json = requests.get("https://raw.githubusercontent.com/Algoritmos-y-Programacion/api-proyecto/main/stadiums.json").json()
    estadios = []
    for estadio_json in estadios_json:
        nombre = estadio_json["name"]
        ubicacion = estadio_json["location"]
        estadios.append(Estadio(nombre, ubicacion))
    return estadios

def cargar_partidos():
    partidos_json = requests.get("https://raw.githubusercontent.com/Algoritmos-y-Programacion/api-proyecto/main/matches.json").json()
    partidos = []
    for partido_json in partidos_json:
        equipo_local_codigo_fifa = partido_json["home_team_fifa_code"]
        equipo_visitante_codigo_fifa = partido_json["away_team_fifa_code"]
        fecha_hora_str = partido_json["date_time"]
        fecha_hora = datetime.datetime.strptime(fecha_hora_str, "%Y-%m-%dT%H:%M:%SZ")
        estadio_nombre = partido_json["stadium_name"]
        partidos.append(Partido(equipo_local_codigo_fifa, equipo_visitante_codigo_fifa, fecha_hora, estadio_nombre))
    return partidos

# Funciones para gestionar los equipos, estadios y partidos
def crear_equipo(nombre, codigo_fifa, grupo):
    equipo = Equipo(nombre, codigo_fifa, grupo)
    equipos.append(equipo)
    guardar_equipos()
    return equipo

def buscar_equipo_por_nombre(nombre):
    for equipo in equipos:
        if equipo.nombre == nombre:
            return equipo
    return None

def buscar_equipo_por_codigo_fifa(codigo_fifa):
    for equipo in equipos:
        if equipo.codigo_fifa == codigo_fifa:
            return equipo
    return None

def crear_estadio(nombre, ubicacion):
    estadio = Estadio(nombre, ubicacion)
    estadios.append(estadio)
    guardar_estadios()
    return estadio

def buscar_estadio_por_nombre(nombre):
    for estadio in estadios:
        if estadio.nombre == nombre:
            return estadio
    return None

def crear_partido(equipo_local, equipo_visitante, fecha_hora, estadio):
    partido = Partido(equipo_local, equipo_visitante, fecha_hora, estadio)
    partidos.append(partido)
    guardar_partidos()
    return partido

def buscar_partido_por_fecha(fecha):
    partidos_filtrados = []
    for partido in partidos:
        if partido.fecha_hora.date() == fecha:
            partidos_filtrados.append(partido)
    return partidos_filtrados

def buscar_partido_por_estadio(estadio):
    partidos_filtrados = []
    for partido in partidos:
        if partido.estadio == estadio:
            partidos_filtrados.append(partido)
    return partidos_filtrados

def buscar_partido_por_equipo(equipo):
    partidos_filtrados = []
    for partido in partidos:
        if partido.equipo_local == equipo or partido.equipo_visitante == equipo:
            partidos_filtrados.append(partido)
    return partidos_filtrados

# Funciones para gestionar las entradas
def crear_cliente(nombre, cedula, edad):
    cliente = Cliente(nombre, cedula, edad)
    clientes.append(cliente)
    guardar_clientes()
    return cliente

def buscar_cliente_por_cedula(cedula):
    for cliente in clientes:
        if cliente.cedula == cedula:
            return cliente
    return None

def crear_entrada(cliente, partido, tipo, asiento, costo):
    entrada = Entrada(cliente, partido, tipo, asiento, costo)
    entradas.append(entrada)
    guardar_entradas()
    return entrada

def calcular_costo_entrada(partido, tipo, cedula):
    costo = 0
    if tipo == "General":
        costo = 35
    elif tipo == "VIP":
        costo = 75
    if es_numero_vampiro(cedula):
        costo *= 0.5
    costo *= 1.16
    return costo

def vender_entrada(cliente, partido, tipo):
    asiento = buscar_asiento_libre(partido.estadio)
    if asiento is None:
        print("No hay asientos disponibles para este partido.")
        return None
    costo = calcular_costo_entrada(partido, tipo, cliente.cedula)
    entrada = crear_entrada(cliente, partido, tipo, asiento, costo)
    asiento.ocupado = True
    actualizar_asiento(asiento)
    return entrada

# Funciones para gestionar los asientos
def crear_asiento(numero, fila, estadio, ocupado=False):
    asiento = Asiento(numero, fila, estadio, ocupado)
    asientos.append(asiento)
    guardar_asientos()
    return asiento

def buscar_asiento_libre(estadio):
    for asiento in asientos:
        if asiento.estadio == estadio and not asiento.ocupado:
            return asiento
    return None

def actualizar_asiento(asiento):
    for i in range(len(asientos)):
        if asientos[i].numero == asiento.numero and asientos[i].fila == asiento.fila and asientos[i].estadio == asiento.estadio:
            asientos[i] = asiento
            break
    guardar_asientos()

# Funciones para gestionar los restaurantes
def crear_producto(nombre, tipo, precio, inventario=10):
    producto = Producto(nombre, tipo, precio, inventario)
    productos.append(producto)
    guardar_productos()
    return producto

def buscar_producto_por_nombre(nombre):
    for producto in productos:
        if producto.nombre == nombre:
            return producto
    return None

def buscar_productos_por_tipo(tipo):
    productos_filtrados = []
    for producto in productos:
        if producto.tipo == tipo:
            productos_filtrados.append(producto)
    return productos_filtrados

def buscar_productos_por_precio(precio_min, precio_max):
    productos_filtrados = []
    for producto in productos:
        if producto.precio >= precio_min and producto.precio <= precio_max:
            productos_filtrados.append(producto)
    return productos_filtrados

def crear_orden_restaurante(cliente, productos):
    costo_total = 0
    for producto in productos:
        costo_total += producto.precio
    orden = OrdenRestaurante(cliente, productos, costo_total)
    ordenes_restaurante.append(orden)
    guardar_ordenes_restaurante()
    return orden

def calcular_descuento_orden(orden):
    if es_numero_perfecto(orden.cliente.cedula):
        return 0.15
    return 0

def realizar_venta_restaurante(cliente, productos):
    orden = crear_orden_restaurante(cliente, productos)
    descuento = calcular_descuento_orden(orden)
    costo_total = orden.costo * (1 - descuento)
    print(f"Su orden tiene un descuento de {descuento * 100}%.")
    print(f"El costo total de su orden es: {costo_total}")
    for producto in productos:
        restar_inventario(producto)

# Funciones para gestionar el inventario
def restar_inventario(producto):
    for i in range(len(productos)):
        if productos[i].nombre == producto.nombre:
            productos[i].inventario -= 1
            break
    guardar_productos()

# Funciones para cargar y guardar datos en archivos
def cargar_equipos_desde_archivo(archivo):
    global equipos
    equipos = []
    with open(archivo, "r") as f:
        equipos_json = json.load(f)
        for equipo_json in equipos_json:
            equipos.append(Equipo(equipo_json["nombre"], equipo_json["codigo_fifa"], equipo_json["grupo"]))

def guardar_equipos():
    equipos_json = []
    for equipo in equipos:
        equipos_json.append({"nombre": equipo.nombre, "codigo_fifa": equipo.codigo_fifa, "grupo": equipo.grupo})
    with open("equipos.json", "w") as f:
        json.dump(equipos_json, f)

def cargar_estadios_desde_archivo(archivo):
    global estadios
    estadios = []
    with open(archivo, "r") as f:
        estadios_json = json.load(f)
        for estadio_json in estadios_json:
            estadios.append(Estadio(estadio_json["nombre"], estadio_json["ubicacion"]))

def guardar_estadios():
    estadios_json = []
    for estadio in estadios:
        estadios_json.append({"nombre": estadio.nombre, "ubicacion": estadio.ubicacion})
    with open("estadios.json", "w") as f:
        json.dump(estadios_json, f)

def cargar_partidos_desde_archivo(archivo):
    global partidos
    partidos = []
    with open(archivo, "r") as f:
        partidos_json = json.load(f)
        for partido_json in partidos_json:
            fecha_hora = datetime.datetime.strptime(partido_json["fecha_hora"], "%Y-%m-%dT%H:%M:%SZ")
            partidos.append(Partido(partido_json["equipo_local"], partido_json["equipo_visitante"], fecha_hora, partido_json["estadio"], partido_json["asistencia"]))

def guardar_partidos():
    partidos_json = []
    for partido in partidos:
        partidos_json.append({"equipo_local": partido.equipo_local, "equipo_visitante": partido.equipo_visitante, "fecha_hora": partido.fecha_hora.isoformat(), "estadio": partido.estadio, "asistencia": partido.asistencia})
    with open("partidos.json", "w") as f:
        json.dump(partidos_json, f)

def cargar_clientes_desde_archivo(archivo):
    global clientes
    clientes = []
    with open(archivo, "r") as f:
        clientes_json = json.load(f)
        for cliente_json in clientes_json:
            clientes.append(Cliente(cliente_json["nombre"], cliente_json["cedula"], cliente_json["edad"]))

def guardar_clientes():
    clientes_json = []
    for cliente in clientes:
        clientes_json.append({"nombre": cliente.nombre, "cedula": cliente.cedula, "edad": cliente.edad})
    with open("clientes.json", "w") as f:
        json.dump(clientes_json, f)

def cargar_entradas_desde_archivo(archivo):
    global entradas
    entradas = []
    with open(archivo, "r") as f:
        entradas_json = json.load(f)
        for entrada_json in entradas_json:
            cliente = buscar_cliente_por_cedula(entrada_json["cliente_cedula"])
            partido = buscar_partido_por_fecha(datetime.datetime.strptime(entrada_json["partido_fecha_hora"], "%Y-%m-%dT%H:%M:%SZ").date())
            asiento_numero = entrada_json["asiento_numero"]
            asiento_fila = entrada_json["asiento_fila"]
            estadio_nombre = entrada_json["asiento_estadio"]
            asiento = buscar_asiento_por_numero(asiento_numero, asiento_fila, estadio_nombre)
            entradas.append(Entrada(cliente, partido, entrada_json["tipo"], asiento, entrada_json["costo"]))

def guardar_entradas():
    entradas_json = []
    for entrada in entradas:
        entradas_json.append({"cliente_cedula": entrada.cliente.cedula, "partido_fecha_hora": entrada.partido.fecha_hora.isoformat(), "tipo": entrada.tipo, "asiento_numero": entrada.asiento.numero, "asiento_fila": entrada.asiento.fila, "asiento_estadio": entrada.asiento.estadio, "costo": entrada.costo})
    with open("entradas.json", "w") as f:
        json.dump(entradas_json, f)

def cargar_asientos_desde_archivo(archivo):
    global asientos
    asientos = []
    with open(archivo, "r") as f:
        asientos_json = json.load(f)
        for asiento_json in asientos_json:
            estadio = buscar_estadio_por_nombre(asiento_json["estadio"])
            asientos.append(Asiento(asiento_json["numero"], asiento_json["fila"], estadio, asiento_json["ocupado"]))

def guardar_asientos():
    asientos_json = []
    for asiento in asientos:
        asientos_json.append({"numero": asiento.numero, "fila": asiento.fila, "estadio": asiento.estadio.nombre, "ocupado": asiento.ocupado})
    with open("asientos.json", "w") as f:
        json.dump(asientos_json, f)

def cargar_productos_desde_archivo(archivo):
    global productos
    productos = []
    with open(archivo, "r") as f:
        productos_json = json.load(f)
        for producto_json in productos_json:
            productos.append(Producto(producto_json["nombre"], producto_json["tipo"], producto_json["precio"], producto_json["inventario"]))

def guardar_productos():
    productos_json = []
    for producto in productos:
        productos_json.append({"nombre": producto.nombre, "tipo": producto.tipo, "precio": producto.precio, "inventario": producto.inventario})
    with open("productos.json", "w") as f:
        json.dump(productos_json, f)

def cargar_ordenes_restaurante_desde_archivo(archivo):
    global ordenes_restaurante
    ordenes_restaurante = []
    with open(archivo, "r") as f:
        ordenes_restaurante_json = json.load(f)
        for orden_restaurante_json in ordenes_restaurante_json:
            cliente = buscar_cliente_por_cedula(orden_restaurante_json["cliente_cedula"])
            productos = []
            for producto_json in orden_restaurante_json["productos"]:
                producto = buscar_producto_por_nombre(producto_json["nombre"])
                productos.append(producto)
            ordenes_restaurante.append(OrdenRestaurante(cliente, productos, orden_restaurante_json["costo"]))

def guardar_ordenes_restaurante():
    ordenes_restaurante_json = []
    for orden_restaurante in ordenes_restaurante:
        productos_json = []
        for producto in orden_restaurante.productos:
            productos_json.append({"nombre": producto.nombre})
        ordenes_restaurante_json.append({"cliente_cedula": orden_restaurante.cliente.cedula, "productos": productos_json, "costo": orden_restaurante.costo})
    with open("ordenes_restaurante.json", "w") as f:
        json.dump(ordenes_restaurante_json, f)

# Funciones para realizar estadísticas
def calcular_promedio_gasto_vip():
    gastos_vip = []
    for entrada in entradas:
        if entrada.tipo == "VIP":
            gastos_vip.append(entrada.costo)
    promedio = sum(gastos_vip) / len(gastos_vip) if gastos_vip else 0
    return promedio

def obtener_estadísticas_partidos():
    partidos_ordenados = sorted(partidos, key=lambda partido: partido.asistencia, reverse=True)
    estadísticas_partidos = []
    for partido in partidos_ordenados:
        equipo_local = buscar_equipo_por_codigo_fifa(partido.equipo_local)
        equipo_visitante = buscar_equipo_por_codigo_fifa(partido.equipo_visitante)
        estadísticas_partidos.append({"partido": f"{equipo_local.nombre} vs {equipo_visitante.nombre}", "estadio": partido.estadio, "boletos_vendidos": partido.asistencia, "asistencia": partido.asistencia})
    return estadísticas_partidos

def obtener_partido_con_mayor_asistencia():
    partido_mayor_asistencia = max(partidos, key=lambda partido: partido.asistencia)
    return partido_mayor_asistencia

def obtener_partido_con_mayor_boletos_vendidos():
    partido_mayor_boletos_vendidos = max(partidos, key=lambda partido: partido.asistencia)
    return partido_mayor_boletos_vendidos

def obtener_productos_mas_vendidos():
    productos_vendidos = {}
    for orden in ordenes_restaurante:
        for producto in orden.productos:
            if producto.nombre in productos_vendidos:
                productos_vendidos[producto.nombre] += 1
            else:
                productos_vendidos[producto.nombre] = 1
    productos_mas_vendidos = sorted(productos_vendidos.items(), key=lambda item: item[1], reverse=True)
    return productos_mas_vendidos[:3]

def obtener_clientes_con_mas_boletos():
    clientes_boletos = {}
    for entrada in entradas:
        if entrada.cliente.cedula in clientes_boletos:
            clientes_boletos[entrada.cliente.cedula] += 1
        else:
            clientes_boletos[entrada.cliente.cedula] = 1
    clientes_mas_boletos = sorted(clientes_boletos.items(), key=lambda item: item[1], reverse=True)
    return clientes_mas_boletos[:3]

def generar_graficos(estadísticas_partidos, productos_mas_vendidos, clientes_mas_boletos):
    # Gráfico de partidos ordenados por asistencia
    partidos = [estadistica["partido"] for estadistica in estadísticas_partidos]
    asistencias = [estadistica["asistencia"] for estadistica in estadísticas_partidos]
    plt.figure(figsize=(10, 5))
    plt.bar(partidos, asistencias)
    plt.title("Partidos ordenados por asistencia")
    plt.xlabel("Partido")
    plt.ylabel("Asistencia")
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.show()

    # Gráfico de productos más vendidos
    productos = [producto[0] for producto in productos_mas_vendidos]
    cantidades = [producto[1] for producto in productos_mas_vendidos]
    plt.figure(figsize=(10, 5))
    plt.pie(cantidades, labels=productos, autopct="%1.1f%%")
    plt.title("Productos más vendidos")
    plt.show()

    # Gráfico de clientes con más boletos
    clientes = [cliente[0] for cliente in clientes_mas_boletos]
    cantidades = [cliente[1] for cliente in clientes_mas_boletos]
    plt.figure(figsize=(10, 5))
    plt.bar(clientes, cantidades)
    plt.title("Clientes con más boletos")
    plt.xlabel("Cliente")
    plt.ylabel("Cantidad de boletos")
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.show()

# Inicialización de variables globales
equipos = []
estadios = []
partidos = []
clientes = []
entradas = []
asientos = []
productos = []
ordenes_restaurante = []

# Cargar datos desde archivos (si existen)
try:
    cargar_equipos_desde_archivo("equipos.json")
except FileNotFoundError:
    print("No se encontró el archivo equipos.json. Se cargarán los equipos desde la API.")
    equipos = cargar_equipos()
    guardar_equipos()

try:
    cargar_estadios_desde_archivo("estadios.json")
except FileNotFoundError:
    print("No se encontró el archivo estadios.json. Se cargarán los estadios desde la API.")
    estadios = cargar_estadios()
    guardar_estadios()

try:
    cargar_partidos_desde_archivo("partidos.json")
except FileNotFoundError:
    print("No se encontró el archivo partidos.json. Se cargarán los partidos desde la API.")
    partidos = cargar_partidos()
    guardar_partidos()

try:
    cargar_clientes_desde_archivo("clientes.json")
except FileNotFoundError:
    print("No se encontró el archivo clientes.json.")

try:
    cargar_entradas_desde_archivo("entradas.json")
except FileNotFoundError:
    print("No se encontró el archivo entradas.json.")

try:
    cargar_asientos_desde_archivo("asientos.json")
except FileNotFoundError:
    print("No se encontró el archivo asientos.json.")

try:
    cargar_productos_desde_archivo("productos.json")
except FileNotFoundError:
    print("No se encontró el archivo productos.json.")

try:
    cargar_ordenes_restaurante_desde_archivo("ordenes_restaurante.json")
except FileNotFoundError:
    print("No se encontró el archivo ordenes_restaurante.json.")

# Menú principal
while True:
    print("\nMenú principal:")
    print("1. Gestión de equipos")
    print("2. Gestión de estadios")
    print("3. Gestión de partidos")
    print("4. Gestión de ventas de entradas")
    print("5. Gestión de restaurantes")
    print("6. Indicadores de gestión")
    print("7. Salir")
    opcion = input("Seleccione una opción: ")

    if opcion == "1":
        # Gestión de equipos
        while True:
            print("\nGestión de equipos:")
            print("1. Crear equipo")
            print("2. Buscar equipo por nombre")
            print("3. Buscar equipo por código FIFA")
            print("4. Volver al menú principal")
            opcion_equipo = input("Seleccione una opción: ")

            if opcion_equipo == "1":
                nombre = input("Ingrese el nombre del equipo: ")
                codigo_fifa = input("Ingrese el código FIFA del equipo: ")
                grupo = input("Ingrese el grupo del equipo: ")
                crear_equipo(nombre, codigo_fifa, grupo)
                print("Equipo creado correctamente.")

            elif opcion_equipo == "2":
                nombre = input("Ingrese el nombre del equipo: ")
                equipo = buscar_equipo_por_nombre(nombre)
                if equipo is not None:
                    print(f"Equipo encontrado: {equipo.nombre}, {equipo.codigo_fifa}, {equipo.grupo}")
                else:
                    print("Equipo no encontrado.")

            elif opcion_equipo == "3":
                codigo_fifa = input("Ingrese el código FIFA del equipo: ")
                equipo = buscar_equipo_por_codigo_fifa(codigo_fifa)
                if equipo is not None:
                    print(f"Equipo encontrado: {equipo.nombre}, {equipo.codigo_fifa}, {equipo.grupo}")
                else:
                    print("Equipo no encontrado.")

            elif opcion_equipo == "4":
                break

            else:
                print("Opción inválida.")

    elif opcion == "2":
        # Gestión de estadios
        while True:
            print("\nGestión de estadios:")
            print("1. Crear estadio")
            print("2. Buscar estadio por nombre")
            print("3. Volver al menú principal")
            opcion_estadio = input("Seleccione una opción: ")

            if opcion_estadio == "1":
                nombre = input("Ingrese el nombre del estadio: ")
                ubicacion = input("Ingrese la ubicación del estadio: ")
                crear_estadio(nombre, ubicacion)
                print("Estadio creado correctamente.")

            elif opcion_estadio == "2":
                nombre = input("Ingrese el nombre del estadio: ")
                estadio = buscar_estadio_por_nombre(nombre)
                if estadio is not None:
                    print(f"Estadio encontrado: {estadio.nombre}, {estadio.ubicacion}")
                else:
                    print("Estadio no encontrado.")

            elif opcion_estadio == "3":
                break

            else:
                print("Opción inválida.")

    elif opcion == "3":
        # Gestión de partidos
        while True:
            print("\nGestión de partidos:")
            print("1. Crear partido")
            print("2. Buscar partidos por fecha")
            print("3. Buscar partidos por estadio")
            print("4. Buscar partidos por equipo")
            print("5. Volver al menú principal")
            opcion_partido = input("Seleccione una opción: ")

            if opcion_partido == "1":
                equipo_local_codigo_fifa = input("Ingrese el código FIFA del equipo local: ")
                equipo_visitante_codigo_fifa = input("Ingrese el código FIFA del equipo visitante: ")
                fecha_hora_str = input("Ingrese la fecha y hora del partido (AAAA-MM-DDTHH:MM:SS): ")
                fecha_hora = datetime.datetime.strptime(fecha_hora_str, "%Y-%m-%dT%H:%M:%S")
                estadio_nombre = input("Ingrese el nombre del estadio: ")
                crear_partido(equipo_local_codigo_fifa, equipo_visitante_codigo_fifa, fecha_hora, estadio_nombre)
                print("Partido creado correctamente.")

            elif opcion_partido == "2":
                fecha_str = input("Ingrese la fecha del partido (AAAA-MM-DD): ")
                fecha = datetime.datetime.strptime(fecha_str, "%Y-%m-%d").date()
                partidos_filtrados = buscar_partido_por_fecha(fecha)
                if partidos_filtrados:
                    for partido in partidos_filtrados:
                        equipo_local = buscar_equipo_por_codigo_fifa(partido.equipo_local)
                        equipo_visitante = buscar_equipo_por_codigo_fifa(partido.equipo_visitante)
                        print(f"Partido: {equipo_local.nombre} vs {equipo_visitante.nombre}, {partido.fecha_hora}, {partido.estadio}")
                else:
                    print("No se encontraron partidos para la fecha indicada.")

            elif opcion_partido == "3":
                estadio_nombre = input("Ingrese el nombre del estadio: ")
                partidos_filtrados = buscar_partido_por_estadio(estadio_nombre)
                if partidos_filtrados:
                    for partido in partidos_filtrados:
                        equipo_local = buscar_equipo_por_codigo_fifa(partido.equipo_local)
                        equipo_visitante = buscar_equipo_por_codigo_fifa(partido.equipo_visitante)
                        print(f"Partido: {equipo_local.nombre} vs {equipo_visitante.nombre}, {partido.fecha_hora}, {partido.estadio}")
                else:
                    print("No se encontraron partidos para el estadio indicado.")

            elif opcion_partido == "4":
                codigo_fifa = input("Ingrese el código FIFA del equipo: ")
                partidos_filtrados = buscar_partido_por_equipo(codigo_fifa)
                if partidos_filtrados:
                    for partido in partidos_filtrados:
                        equipo_local = buscar_equipo_por_codigo_fifa(partido.equipo_local)
                        equipo_visitante = buscar_equipo_por_codigo_fifa(partido.equipo_visitante)
                        print(f"Partido: {equipo_local.nombre} vs {equipo_visitante.nombre}, {partido.fecha_hora}, {partido.estadio}")
                else:
                    print("No se encontraron partidos para el equipo indicado.")

            elif opcion_partido == "5":
                break

            else:
                print("Opción inválida.")

    elif opcion == "4":
        # Gestión de ventas de entradas
        while True:
            print("\nGestión de ventas de entradas:")
            print("1. Registrar cliente")
            print("2. Vender entrada")
            print("3. Volver al menú principal")
            opcion_entrada = input("Seleccione una opción: ")

            if opcion_entrada == "1":
                nombre = input("Ingrese el nombre del cliente: ")
                cedula = int(input("Ingrese la cédula del cliente: "))
                edad = int(input("Ingrese la edad del cliente: "))
                crear_cliente(nombre, cedula, edad)
                print("Cliente registrado correctamente.")

            elif opcion_entrada == "2":
                cedula = int(input("Ingrese la cédula del cliente: "))
                cliente = buscar_cliente_por_cedula(cedula)
                if cliente is None:
                    print("Cliente no encontrado.")
                    continue

                fecha_str = input("Ingrese la fecha del partido (AAAA-MM-DD): ")
                fecha = datetime.datetime.strptime(fecha_str, "%Y-%m-%d").date()
                partidos_filtrados = buscar_partido_por_fecha(fecha)
                if not partidos_filtrados:
                    print("No se encontraron partidos para la fecha indicada.")
                    continue

                for i, partido in enumerate(partidos_filtrados):
                    equipo_local = buscar_equipo_por_codigo_fifa(partido.equipo_local)
                    equipo_visitante = buscar_equipo_por_codigo_fifa(partido.equipo_visitante)
                    print(f"{i+1}. Partido: {equipo_local.nombre} vs {equipo_visitante.nombre}, {partido.fecha_hora}, {partido.estadio}")

                opcion_partido = int(input("Seleccione el número del partido: "))
                if 1 <= opcion_partido <= len(partidos_filtrados):
                    partido = partidos_filtrados[opcion_partido - 1]
                    tipo = input("Ingrese el tipo de entrada (General/VIP): ").capitalize()
                    if tipo in ["General", "VIP"]:
                        entrada = vender_entrada(cliente, partido, tipo)
                        if entrada is not None:
                            print(f"Entrada vendida correctamente. Tipo: {entrada.tipo}, Asiento: {entrada.asiento.numero}, Fila: {entrada.asiento.fila}, Costo: {entrada.costo}")
                            partido.asistencia += 1
                            actualizar_partido(partido)
                    else:
                        print("Tipo de entrada inválido.")
                else:
                    print("Opción inválida.")

            elif opcion_entrada == "3":
                break

            else:
                print("Opción inválida.")

    elif opcion == "5":
        # Gestión de restaurantes
        while True:
            print("\nGestión de restaurantes:")
            print("1. Crear producto")
            print("2. Buscar producto por nombre")
            print("3. Buscar productos por tipo")
            print("4. Buscar productos por precio")
            print("5. Realizar venta de restaurante")
            print("6. Volver al menú principal")
            opcion_restaurante = input("Seleccione una opción: ")

            if opcion_restaurante == "1":
                nombre = input("Ingrese el nombre del producto: ")
                tipo = input("Ingrese el tipo del producto (alimento/bebida): ").capitalize()
                precio = float(input("Ingrese el precio del producto: "))
                crear_producto(nombre, tipo, precio)
                print("Producto creado correctamente.")

            elif opcion_restaurante == "2":
                nombre = input("Ingrese el nombre del producto: ")
                producto = buscar_producto_por_nombre(nombre)
                if producto is not None:
                    print(f"Producto encontrado: {producto.nombre}, {producto.tipo}, {producto.precio}")
                else:
                    print("Producto no encontrado.")

            elif opcion_restaurante == "3":
                tipo = input("Ingrese el tipo del producto (alimento/bebida): ").capitalize()
                productos_filtrados = buscar_productos_por_tipo(tipo)
                if productos_filtrados:
                    for producto in productos_filtrados:
                        print(f"Producto: {producto.nombre}, {producto.tipo}, {producto.precio}")
                else:
                    print("No se encontraron productos para el tipo indicado.")

            elif opcion_restaurante == "4":
                precio_min = float(input("Ingrese el precio mínimo: "))
                precio_max = float(input("Ingrese el precio máximo: "))
                productos_filtrados = buscar_productos_por_precio(precio_min, precio_max)
                if productos_filtrados:
                    for producto in productos_filtrados:
                        print(f"Producto: {producto.nombre}, {producto.tipo}, {producto.precio}")
                else:
                    print("No se encontraron productos para el rango de precios indicado.")

            elif opcion_restaurante == "5":
                cedula = int(input("Ingrese la cédula del cliente: "))
                cliente = buscar_cliente_por_cedula(cedula)
                if cliente is None:
                    print("Cliente no encontrado.")
                    continue

                productos_seleccionados = []
                while True:
                    nombre = input("Ingrese el nombre del producto (o ingrese 'listar' para ver la lista de productos): ")
                    if nombre.lower() == "listar":
                        productos_filtrados = buscar_productos_por_tipo("alimento") + buscar_productos_por_tipo("bebida")
                        if productos_filtrados:
                            for i, producto in enumerate(productos_filtrados):
                                print(f"{i+1}. {producto.nombre} ({producto.tipo}): {producto.precio}")
                        else:
                            print("No se encontraron productos.")
                    else:
                        producto = buscar_producto_por_nombre(nombre)
                        if producto is not None:
                            productos_seleccionados.append(producto)
                            print(f"Producto {nombre} agregado al carrito.")
                        else:
                            print("Producto no encontrado.")

                    continuar = input("¿Desea agregar otro producto al carrito? (s/n): ").lower()
                    if continuar != "s":
                        break

                realizar_venta_restaurante(cliente, productos_seleccionados)

            elif opcion_restaurante == "6":
                break

            else:
                print("Opción inválida.")

    elif opcion == "6":
        # Indicadores de gestión
        while True:
            print("\nIndicadores de gestión:")
            print("1. Promedio de gasto de clientes VIP en un partido")
            print("2. Estadísticas de partidos")
            print("3. Productos más vendidos")
            print("4. Clientes con más boletos")
            print("5. Generar gráficos")
            print("6. Volver al menú principal")
            opcion_indicadores = input("Seleccione una opción: ")

            if opcion_indicadores == "1":
                print(f"Promedio de gasto de clientes VIP en un partido: {calcular_promedio_gasto_vip()}")

            elif opcion_indicadores == "2":
                estadisticas_partidos = obtener_estadisticas_partidos()
                for estadistica in estadisticas_partidos:
                    print(estadistica)

            elif opcion_indicadores == "3":
                productos_mas_vendidos = obtener_productos_mas_vendidos()
                for i, producto in enumerate(productos_mas_vendidos):
                    print(f"{i+1}. {producto[0]}: {producto[1]} unidades")

            elif opcion_indicadores == "4":
                clientes_mas_boletos = obtener_clientes_con_mas_boletos()
                for i, cliente in enumerate(clientes_mas_boletos):
                    print(f"{i+1}. {cliente[0]}: {cliente[1]} boletos")

            elif opcion_indicadores == "5":
                estadisticas_partidos = obtener_estadisticas_partidos()
                productos_mas_vendidos = obtener_productos_mas_vendidos()
                clientes_mas_boletos = obtener_clientes_con_mas_boletos()
                generar_graficos(estadisticas_partidos, productos_mas_vendidos, clientes_mas_boletos)

            elif opcion_indicadores == "6":
                break

            else:
                print("Opción inválida.")

    elif opcion == "7":
        # Salir
        print("¡Hasta luego!")
        break

    else:
        print("Opción inválida.")